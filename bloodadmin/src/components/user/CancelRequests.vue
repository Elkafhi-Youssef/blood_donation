<template>
<div v-if="!cancelRequests || !cancelRequests.length">
  <h1>no request canceled </h1>
</div>
  <div v-for="item in cancelRequests" key="item.request_id" class="  w-[20rem] shadow-lg border-l-2 border-second_col rounded-lg ">
...
export default {
  ...
  computed:{
    cancelRequests(){
      this.cancelRequests = this.$store.state.cancelRequests
      console.log(this.cancelRequests);
      return this.$store.state.cancelRequests
    }
  },
  async mounted() {
   ...
   await this.$store.dispatch('getCancelRequests',{id:id,token:token})
